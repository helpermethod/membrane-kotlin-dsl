...
fun generateFun(field: ParameterSpec, methodName: String, pattern: String, type: Type): FunSpec {
    ...
    return FunSpec.builder(type.getAnnotation(MCElement::class.java).name).run {
        ...
        if (hasChildren) {
            addParameter(
                    ParameterSpec.builder("init", LambdaTypeName.get(receiver = ClassName("com.predic8.membrane.dsl", "${type.simpleName}Spec"), returnType = Unit::class.asTypeName()))
                            .defaultValue("{}")
                            .build()
            )
        }
        ...
        attrs.forEach { (methodName, parameter) ->
            addStatement("%N.%N(%N)", subTypeName, methodName, parameter)
        }
        if (hasChildren) {
...
fun invokeField(type: Type, fieldName: String) =
        tryInvokeField(type, fieldName).fold({ _ -> null }) { v -> v }
